    ...
    const updateNote = (noteId, data) => {
        ...
        .then(() => {
            if(note.noteType === "TermGoals"){
                ...
                .update({termGoals: arrayRemove(previousTermGoals)})
                .then(() => {
                    db.collection("users").doc(currentUser.uid)
                    .update({termGoals: arrayUnion(termGoals)})
                })
            } else if(note.noteType === "Assessment"){
                ...
                db.collection("users").doc(currentUser.uid)
                .update({nextTarget: arrayRemove(previousAssessment)})
                .then(() => {
                    db.collection("users").doc(currentUser.uid)
                    .update({nextTarget: arrayUnion(nextAssessment)})
                })              
            } else if(note.noteType === "ActionItem"){
                ...
                .update({notes: arrayRemove(previousActionItem)})
                .then(() => {
                    db.collection("users").doc(currentUser.uid).collection('userLists').doc("notesList")
                    .update({notes: arrayUnion(nextActionItem)})
                })    
            }
        })
        .then(()=>{
